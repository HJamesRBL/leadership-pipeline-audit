  ...
  try {
    ...
    for (const leader of incompleteLeaders) {
      try {
        ...
        if (!resend) {
          console.log(`Email service not configured. Would send reminder to ${email} with link: ${auditLink}`);
          emailResults.push({
            leaderName: name,
            email: email,
            status: 'simulated',
            auditLink: auditLink,
            note: 'Email service not configured - link generated but reminder not sent'
          });
          continue;
        }
